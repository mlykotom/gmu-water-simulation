...
CCollisionGeometry::~CCollisionGeometry() = default;
...
QVector3D CCollisionGeometry::inverseBoundingBoxBounce(QVector3D &pos, QVector3D &velocity)
{
    ...
    for (sWall wall : m_boundingBox.m_walls) {
        QVector3D inverseNormal(QVector3D(wall.normal.x, wall.normal.y, wall.normal.z) * (-1));

        double d = QVector3D::dotProduct(QVector3D(wall.position.x, wall.position.y, wall.position.z) - pos, inverseNormal) + 0.01; // particle radius
